#!/usr/bin/env python3
# 旭川リアルタイム情報局 ニュース取得スクリプト
# 市・観光協会・気象庁のRSS/XMLから見出し+日付+リンク+出典だけを集めて docs/news.json に保存する。
# 全文コピーはしない(著作権対策)。
...
def parse_rss(xml_bytes, source_name, max_items=15):
    ...
    for it in channel.findall("item")[:max_items]:
        ...
        if pub:
            try:
                iso = to_iso(parsedate_to_datetime(pub))
            except Exception:
                iso = ""
        if title and link:
            items.append({"title": title, "link": link, "date": iso, "source": source_name})
    return items
...
def parse_jma(xml_bytes, max_items=6):
    ...
    for entry in root.findall(f"{ATOM}entry"):
        ...
        if author != "旭川地方気象台":
            continue
        if not any(k in title for k in JMA_KEEP):
            continue
        if "上川" not in content and "上川" not in title:
            continue
        ...
        if updated:
            try:
                iso = to_iso(datetime.datetime.fromisoformat(updated.replace("Z","+00:00")))
            except Exception:
                iso = ""
        items.append({"title": disp, "link": link, "date": iso, "source": "気象庁"})
...
def main():
    all_items = []
    sources = [
        ("https://www.city.asahikawa.hokkaido.jp/700/news.xml", "旭川市", parse_rss),
        ("https://www.atca.jp/feed/", "旭川観光協会", parse_rss),
    ]
    for url, name, parser in sources:
        try:
            all_items += parser(fetch(url), name)
            print(f"[OK] {name}: 取得成功", file=sys.stderr)
    ...
    for url in [
        ...
        try:
            ...
            print(f"[OK] 気象庁 {url.split('/')[-1]}: 取得成功", file=sys.stderr)
        except Exception as e:
            print(f"[NG] 気象庁 {url}: {e}", file=sys.stderr)

    all_items.sort(key=lambda x: x["date"] or "", reverse=True)
    out = {
        "updated": to_iso(datetime.datetime.now(datetime.timezone.utc)),
        "items": all_items,
    }
